...
from tensorflow.keras.layers import SimpleRNN, Dense, InputLayer, Dropout
...
print(f"RNN Model RMSE: {rmse:.3f}")

plt.figure(figsize=(12, 6))
plt.plot(y_test.values, label='Gerçek Değerler', color='black')
plt.plot(y_pred, label=f'RNN Tahminleri (RMSE={rmse:.3f})', linestyle='--')
plt.legend()
plt.grid(True)
plt.show()